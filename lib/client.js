...
const deprecatedParams = deprecate(() => {}, '#resource() "params" option is deprecated');
...
<body onload="javascript:document.forms[0].submit()">
  ...
  async resource(resourceUrl, accessToken, { params: deprecated, ...options } = {}) {
    let token = accessToken;
    const opts = merge({
      verb: 'GET',
      via: 'header',
      form: 'json' in options ? undefined : true,
    }, options);
    ...
    if (deprecated) {
      ...
      if (verb === 'POST' || verb === 'PATCH' || verb === 'PUT') {
        opts.body = deprecated;
      } else {
        opts.query = deprecated;
      }
    }
    ...
    switch (opts.via) {
      case 'query':
        defaultsDeep(requestOpts, { query: { access_token: token } });
        break;
      case 'body':
        defaultsDeep(requestOpts, { body: { access_token: token } });
        break;
      case 'header':
        ...
        break;
      default:
        throw new TypeError('opts.via must be either "header", "query", or "body"');
    }
  ...
  async userinfo(accessToken, {
    ...
    const options = { verb, via, tokenType };
    options.verb = String(options.verb).toUpperCase();

    if (options.verb !== 'GET' && options.verb !== 'POST') {
      throw new TypeError('#userinfo() verb can only be POST or a GET');
    }

    if (via === 'query' && options.verb !== 'GET') {
      throw new TypeError('userinfo endpoints will only parse query strings for GET requests');
    } else if (via === 'body' && options.verb !== 'POST') {
