...
describe('Client', () => {
  ...
  describe('#endSessionUrl', function () {
    ...
    it('when TokenSet is passed in it MUST have an id_token', function () {
      ...
      expect(() => this.client.endSessionUrl({
        id_token_hint: hint,
  ...
  describe('#callback', function () {
    ...
    it('pushes default_max_age to #validateIdToken', function () {
      ...
      return client.callback('https://rp.example.com/cb', {
        code: 'codeValue',
      })
        .then(fail, () => {
    ...
    it('resolves a tokenset with just a state for response_type=none', function () {
      ...
      return this.client.callback('https://rp.example.com/cb', state, state)
        .then((set) => {
    ...
    describe('state checks', function () {
      ...
      it('rejects with an Error when states mismatch (general mismatch)', function () {
        return this.client.callback('https://rp.example.com/cb', {
          ...
          .then(fail, (error) => {
            expect(error).to.be.instanceof(Error);
            expect(error).to.have.property('message').that.matches(/^state mismatch, expected \S+, got: \S+$/);
          });
      });
    });
  ...
  describe('#userinfo', function () {
    ...
    it('takes a tokenset and validates the subject in id_token is the same in userinfo', function () {
      ...
      return client.userinfo(new TokenSet({
        ...
        expect(nock.isDone()).to.be.true;
        expect(err.message).to.equal('userinfo sub mismatch, expected subject, got: different-subject');
      });
    });
    ...
    describe('signed response (content-type = application/jwt)', function () {
      ...
      it('validates the response is a application/jwt', function () {
        ...
        return client.userinfo().then(fail, (err) => {
          expect(err.message).to.eql('expected application/jwt response from the userinfo_endpoint');
        });
      });
    });
  });
      ...
      describe('alg resolution', () => {
        it('rejects when no valid key is present', () => {
          ...
          return keystore.generate('EC', 'P-256').then(() => {
            ...
            return clientInternal.authFor.call(client, 'token')
              .then(fail, (err) => {
                expect(err).to.have.property('message', 'no key found in client jwks to sign a client assertion with using alg EdDSA');
              });
          });
        });
      });
  ...
  describe('Client#validateIdToken', function () {
    ...
    it('validates state presence when s_hash is returned', function () {
      ...
      return this.IdToken(this.keystore.get(), 'RS256', {
        ...
        .then(fail, (error) => {
          expect(error).to.have.property('message', 'cannot verify s_hash, "checks.state" property not provided');
        });
    });
    ...
    it('validates s_hash', function () {
      ...
      return this.IdToken(this.keystore.get(), 'RS256', {
        ...
        .then(fail, (error) => {
          expect(error).to.have.property('message').that.matches(/^s_hash mismatch, expected \S+, got: \S+$/);
        });
    });
    ...
    it('fails with the wrong at_hash', function () {
      ...
      return this.IdToken(this.keystore.get(), 'RS256', {
        ...
        .then(fail, (error) => {
          expect(error).to.have.property('message').that.matches(/^at_hash mismatch, expected \S+, got: \S+$/);
        });
    });
    ...
    it('fails with the wrong c_hash', function () {
      ...
      return this.IdToken(this.keystore.get(), 'RS256', {
        ...
        .then(fail, (error) => {
          expect(error).to.have.property('message').that.matches(/^c_hash mismatch, expected \S+, got: \S+$/);
        });
    });
  ...
  describe('Distributed and Aggregated Claims', function () {
    ...
    describe('Client#fetchDistributedClaims', function () {
      ...
      it('rejects invalid inputs', function () {
        return this.client.fetchDistributedClaims(true)
          .then(fail, (error) => {
            expect(error).to.be.instanceof(TypeError);
            expect(error.message).to.eql('claims argument must be a plain object');
          });
      });
    ...
    describe('Client#unpackAggregatedClaims', function () {
      ...
      it('rejects invalid inputs', function () {
        return this.client.unpackAggregatedClaims(true)
          .then(fail, (error) => {
            expect(error).to.be.instanceof(TypeError);
            expect(error.message).to.eql('claims argument must be a plain object');
          });
